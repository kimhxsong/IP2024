...
def main():
    ...
    shape = round(cap1.get(cv2.CAP_PROP_FRAME_WIDTH)), round(cap1.get(cv2.CAP_PROP_FRAME_HEIGHT))
    fourcc = cv2.VideoWriter.fourcc(*'XVID')  # Changed codec to 'mp4v'
    output = cv2.VideoWriter('./lec5_video_output.avi', fourcc, fps, shape)  # Changed file extension to .mp4
